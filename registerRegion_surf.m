...
function registerRegion_surf(Parent, marker_tform, fpath, filename, image, k, nm, pixel_region, pixel_region_buff)
    ...
    for t=1:length(nm) %for each region
        ...
        for z = 1:length(filename) %for each marker file
            ...
            while isempty(find(~cellfun(@isempty,cont),1))==0 || isempty(find(~cellfun(@isempty,next), 1)) == 0 || isempty(find(~cellfun(@isempty,cont2),1))==0
               continue
            end

            wObj = imread(fpath{z},'PixelRegion', pixel_region_buff{z});
 ...
 %apply slide transform- needs a scale factor to apply L4 to L1
            ...
            Reg_wObj = cat(3, warped{1}, warped{2}, warped{3}); %combine all RGB warps for final image
            Obj1=Reg_wObj(:,:,3);
            ...
            [tform_reg, inlierDistorted, ~, status] = estimateGeometricTransform(...
            ...
            lastwarn('');
            new_warped = cell(1, length(channel));
            if status == 0
                for u=1:length(channel)
                       new_warped{u} = imwarp(channel{u}, tform_reg, 'OutputView', outputView1);
                end
            end
            [~, msgid] = lastwarn;
    ...
    %ODD SIZING- need to check region_registered, pixel_region etc.
            region_Registered = cat(3, new_warped{1}, new_warped{2}, new_warped{3}); %combine all RGB warps for final image
            [height,width] =size(region_Registered);
      ...
      %YOU AR HEREE! CROP pixel_region -50 in R format          
                reg_crop = imcrop(region_Registered, [0,0, height, width]);
