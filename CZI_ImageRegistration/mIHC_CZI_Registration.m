...
for a = 3:length(imageset)
    if imageset(a).name == "Registration_Check"
     ...
     [D, xml_files, czifiles, fpath, filename, numfiles] = get_CZimage_sets(imageset(a), checkname);
     [pos]= parse_czAnnotations(xml_files);
     [imagen, maxcol, maxrow, pixel_region_buff, cropregion, idx, rois, k,nm] = sort_CZregions(D, filename, pos, xml_files, fpath);
     register_SURF(Parent,fpath, nm, D, filename,cropregion, pixel_region_buff, imagen, k);
    end
end
